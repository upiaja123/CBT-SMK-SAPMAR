<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('question_banks.index') }}" class="text-gray-400 hover:text-sapta-600 transition">
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('question_banks.store') }}">
                        @csrf
                        
                        <div class="space-y-6">
                            <div>
                                <x-input-label for="name" :value="__('Nama Bank Soal *')" />
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus placeholder="Contoh: UTS Matematika Ganjil" />
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1">Hanya admin/kurikulum yang dapat menentukan pengampu bank soal.</p>
                            </div>
                            @endif

                            <div>
                                <x-input-label for="description" :value="__('Deskripsi (Opsional)')" />
                                <textarea id="description" name="description" rows="4" class="border-gray-300 focus:border-sapta-500 focus:ring-sapta-500 rounded-md shadow-sm block mt-1 w-full" placeholder="Tuliskan deskripsi atau catatan singkat mengenai bank soal ini">{{ old('description') }}</textarea>
                                <x-input-error :messages="$errors->get('description')" class="mt-2" />
                            </div>

                            <div class="flex items-center justify-end mt-4 pt-4 border-t border-gray-100 gap-3">
                                <a href="{{ route('question_banks.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200">Batal</a>
